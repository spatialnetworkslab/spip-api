...
export default function setupApp ({ mongoPath, enableJwt, jwtSettings, dataSources }) {
  ...
  setupRoutes(app, checkJwt, mongoose, dataSources)
...
function setupRoutes (app, checkJwt, mongoose, dataSources) {
  ...
  app.use(...routeArgs(checkJwt, edges(dataSources.edges)))
  app.use(...routeArgs(checkJwt, nodes(dataSources.nodes)))
